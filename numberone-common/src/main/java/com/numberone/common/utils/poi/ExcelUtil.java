...
public class ExcelUtil<T> {
     ...
     * @Param list 导出数据集合, sheetName 工作表的名称, tClass模板对象的Class对象
     * @Return
     * @Exception
     * @Description 对list<Map></>数据源将其里面的数据导入到excel表单
     */
    public AjaxResult exportExcelFromMap(List<Map<String, Object>> list, String sheetName, Class tClass) {
        if (list == null) {
            throw new NullPointerException("list which will be export to secel is null!");
        }
        int listLength = list.size();
        List<T> resultList = new LinkedList<T>();
        for (int i = 0; i < listLength; i++) {
            Map m = list.get(i);
            //写在for循环中虽然降低效率，但不希望一个对象的生成失败影响整个excel 的导出
            try {
                T targetObj = (T) BeanUtils.getBeanFromMap(m, tClass);
                resultList.add(targetObj);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        this.init(resultList, sheetName, Type.EXPORT);
        return exportExcel();
    }
